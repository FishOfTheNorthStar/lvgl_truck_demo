                                             ...
                                             lv_opengl_shader_manager_t * shaders)
{
    ...
    if(model->asset.defaultScene.has_value()) {
        LV_LOG_INFO("Default scene = #%d", model->asset.defaultScene.value());
    }
                                   ...
                                   GLint * primitive_tex_uv_id)
{
    ...
    if(!injest_check_any_image_index_valid(texture)) {
		std::size_t t_textureIndex = material_prop.textureIndex;
		std::size_t t_samplerIndex = texture.samplerIndex.has_value() ? texture.samplerIndex.value() : 9999;
		std::size_t t_imageIndex = texture.imageIndex.has_value() ? texture.imageIndex.value() : 9999;
		std::size_t t_basisuImageIndex = texture.basisuImageIndex.has_value() ? texture.basisuImageIndex.value() : 9999;
		std::size_t t_ddsImageIndex = texture.ddsImageIndex.has_value() ? texture.ddsImageIndex.value() : 9999;
		std::size_t t_webpImageIndex = texture.webpImageIndex.has_value() ? texture.webpImageIndex.value() : 9999;
        const char * has_texture_transform = material_prop.transform != nullptr ? "True" : "False";
        LV_LOG("load_mesh_texture_impl - Texture name = '%s', texture_index = %u, has_texture_transform = %s, samplerIndex = %u, imageIndex = %u, basisuImageIndex = %u, ddsImageIndex = %u, webpImageIndex = %u failed any-index-valid test\n", texture.name.c_str(), t_textureIndex, has_texture_transform, t_samplerIndex, t_imageIndex, t_basisuImageIndex, t_ddsImageIndex, t_webpImageIndex);
        *primitive_tex_prop = 9999;
        return;
    } 
    #if 0
    else {
		...
		std::size_t t_webpImageIndex = texture.webpImageIndex.has_value() ? texture.webpImageIndex.value() : 9999;
        const char * has_texture_transform = material_prop.transform != nullptr ? "True" : "False";
        LV_LOG("load_mesh_texture_impl - Texture name = '%s', texture_index = %u, has_texture_transform = %s, samplerIndex = %u, imageIndex = %u, basisuImageIndex = %u, ddsImageIndex = %u, webpImageIndex = %u OK\n", texture.name.c_str(), t_textureIndex, has_texture_transform, t_samplerIndex, t_imageIndex, t_basisuImageIndex, t_ddsImageIndex, t_webpImageIndex);
    }
    #endif 
                  ...
                  uint32_t index)
{
    ...
    //} else if (index > 9){
    //    lv_snprintf(tmp, sizeof(tmp), "%s_img_0%u", data->filename, index);
    //} else {
        //lv_snprintf(tmp, sizeof(tmp), "%s_img_%u", data->filename, index);
    //}
    uint32_t suffix = 0;
    ...
    std::visit(fastgltf::visitor{
        ...
        {
            suffix = lv_opengl_shader_hash(file_path.uri.path().cbegin());
        },
        [&](fastgltf::sources::Array & vector)
        {
            suffix = vector.bytes.size();
        },
        [&](fastgltf::sources::BufferView & view)
        {
            suffix = data->asset.bufferViews[view.bufferViewIndex].byteLength;
        },
    }, image.data);


    if (image.name != "") {
        lv_snprintf(tmp, sizeof(tmp), "%s_img_%u", image.name.c_str(), suffix);
    } else {
        lv_snprintf(tmp, sizeof(tmp), "%s_img_%u", data->filename, index);
    }
    ...
    if(texture_id != GL_NONE) {
        LV_LOG_TRACE("CACHED (%s) Image (%s) [%d] [%u] [%u]\n", tmp, image.name.c_str(), texture_id, index, hash);
                                          ...
                                          GLuint texture_id)
{
    ...
    LV_LOG_INFO("Unpacking image bufferView from %d bytes\n", buffer_view.byteLength);
    return std::visit(
    fastgltf::visitor{
